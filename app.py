...
app = Flask(__name__)
app.config["SECRET_KEY"] = "cgscsgftvwfg"
...
def connect(auth):
    ...
    print(f"{name} has joined the room {room} members: {rooms[room]["members"]}")
...
def disconnect():
    ...
    if room in rooms:
        print(f"{name} has left the room {room} members: {rooms[room]["members"]}")
    else:
        print(f"{name} has left the room {room} members: 0")
